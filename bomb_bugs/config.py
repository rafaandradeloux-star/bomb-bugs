WIDTH, HEIGHT = 960, 540
BACKGROUND = (135, 206, 235)  # sky blue
SQUARE_COLOR = (78, 196, 84)
ENEMY_COLOR = (180, 70, 70)
TEXT_COLOR = (20, 20, 20)
PLATFORM_COLOR = (205, 170, 125)  # light brown
SLASH_COLOR = (255, 255, 255)

SQUARE_SIZE = 64
ENEMY_SIZE = 56
MAX_HP = 10
ENEMY_MAX_HP = 20

SPEED = 420  # pixels/second
ENEMY_SPEED = 120  # pixels/second
ENEMY_CHASE_SPEED = 185  # pixels/second
ENEMY_JUMP_VELOCITY = -620  # pixels/second
ENEMY_JUMP_COOLDOWN = 0.55  # seconds
ENEMY_HIT_FLASH_DURATION = 0.45  # seconds
ENEMY_PATH_SAMPLE_INTERVAL = 0.04  # seconds
ENEMY_PATH_MAX_POINTS = 80
GRAVITY = 1700  # pixels/second^2
JUMP_VELOCITY = -700  # pixels/second
FLOOR_HEIGHT = 80

DASH_SPEED = 980  # pixels/second
DASH_DURATION = 0.14  # seconds
DASH_COOLDOWN = 0.45  # seconds
TRAIL_LIFETIME = 0.12  # seconds
TRAIL_SPAWN_INTERVAL = 0.08  # seconds (lower trail FPS = choppier look)

SLASH_RANGE_X = 110
SLASH_HEIGHT = 70
SLASH_ACTIVE_TIME = 0.08  # seconds
PLAYER_SLASH_COOLDOWN = 0.20  # seconds
PLAYER_SLASH_KNOCKBACK_SPEED = 620  # pixels/second
SPIDER_POISON_TOTAL_HITS = 3
SPIDER_POISON_TICK_INTERVAL = 0.55  # seconds
ENEMY_KNOCKBACK_DECEL = 2200  # pixels/second^2
ENEMY_HIT_SLOW_MIN = 0.35
ENEMY_HIT_SLOW_RECOVERY = 1.6  # per second
ENEMY_SLASH_COOLDOWN = 1.05  # seconds
ENEMY_BOMB_COOLDOWN = 6.0  # seconds
ENEMY_BOMB_HOMING_SPEED = 700  # pixels/second
MOON_MIN_RADIUS = 14
MOON_MAX_RADIUS = 60
HEAL_AMOUNT = 2
HEAL_COOLDOWN = 5.0  # seconds
HEAL_SPLASH_TIME = 0.42  # seconds
BOMB_DAMAGE = 5
BOMB_COOLDOWN = 5.0  # seconds
BOMB_SPEED_X = 520  # pixels/second
BOMB_SPEED_Y = -360  # pixels/second
BOMB_RECOIL_DISTANCE = 26  # pixels
BOMB_GRAVITY = 1100  # pixels/second^2
BOMB_RADIUS = 14
BOMB_HITBOX_RADIUS = 20
BOMB_KNOCKBACK_SPEED = 540  # pixels/second
BOMB_LIFETIME = 2.4  # seconds
BOMB_TRAIL_SPAWN_INTERVAL = 0.04  # seconds
BOMB_HOMING_SPEED = 980  # pixels/second
WEB_PROJECTILE_SPEED = 1050  # pixels/second
WEB_PROJECTILE_RADIUS = 10
WEB_PROJECTILE_HITBOX_RADIUS = 20
WEB_PROJECTILE_LIFETIME = 1.4  # seconds
WEB_UNWRAP_BLIP_DURATION = 0.14  # seconds
MUSHROOM_CLOUD_LIFETIME = 1.1  # seconds
SCREEN_SHAKE_DURATION = 0.28  # seconds
SCREEN_SHAKE_INTENSITY = 5.0  # pixels
FLOATING_TEXT_LIFETIME = 0.7  # seconds
FLOATING_TEXT_RISE_SPEED = 70  # pixels/second
GROUND_POUND_DAMAGE = 5
GROUND_POUND_FALL_SPEED = 1450  # pixels/second
GROUND_POUND_COOLDOWN = 5.0  # seconds
GROUND_SPIKE_LIFETIME = 0.28  # seconds
GROUND_DENT_LIFETIME = 0.75  # seconds
GROUND_DENT_MAX_DEPTH = 18  # pixels
RUBBLE_COUNT = 16
RUBBLE_GRAVITY = 1200  # pixels/second^2
RUBBLE_LIFETIME_MIN = 0.35  # seconds
RUBBLE_LIFETIME_MAX = 0.7  # seconds
RUBBLE_SPEED_X = 290  # pixels/second
RUBBLE_SPEED_Y = 680  # pixels/second

RESPAWN_DELAY = 2.0  # seconds
RESPAWN_ANIM_TIME = 0.9  # seconds
DUST_COUNT = 140
DUST_LIFETIME_MIN = 0.7  # seconds
DUST_LIFETIME_MAX = 1.5  # seconds
DUST_DRIFT_X = 140  # pixels/second
DUST_DRIFT_Y = 90  # pixels/second

HEALTH_BAR_WIDTH = 64
HEALTH_BAR_HEIGHT = 8

PLAYER_RESPAWN_TEXT_Y = 208
ENEMY_RESPAWN_TEXT_Y = 240
